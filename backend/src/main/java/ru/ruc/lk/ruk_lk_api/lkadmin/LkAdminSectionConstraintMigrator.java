 ...
 * ElementCollection. При добавлении значения в {@link LkAdminSection} старый CHECK роняет старт
...
public class LkAdminSectionConstraintMigrator implements ApplicationRunner {
    ...
    private static final String COLUMN = "section";
    private static final String CONSTRAINT = "lk_admin_user_section_section_check";

    private final JdbcTemplate jdbc;

    public LkAdminSectionConstraintMigrator(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public void run(ApplicationArguments args) {
        ...
        String allowed = Arrays.stream(LkAdminSection.values())
            .map(Enum::name)
            .map(v -> "'" + v.replace("'", "''") + "'")
            .collect(Collectors.joining(", "));

        String desiredDef = "CHECK (((section)::text = ANY ((ARRAY[" + allowed + "])::text[])))";
        ...
        for (String name : findSectionCheckNames()) {
    ...
    private boolean isPostgreSql() {
        ...
        try (var connection = jdbc.getDataSource().getConnection()) {
            ...
            log.warn("Не удалось определить СУБД для миграции CHECK: {}", e.getMessage());
            return false;
        }
    }
    ...
    private List<String> findSectionCheckNames() {
        return jdbc.query(
            """
            SELECT c.conname
            FROM pg_constraint c
