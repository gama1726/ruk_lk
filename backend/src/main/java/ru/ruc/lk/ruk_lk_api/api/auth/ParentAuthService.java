...
public class ParentAuthService {
    ...
    /** Шаг 1: зачётка → список ролей родителей без ФИО. */
    ...
    public MaxBindLinkResponse maxBindLink(HttpSession session) {
        ...
        MaxBindingService.MaxBindLink link = maxBindingService.createBindLink(
    ...
    public ParentDeliveryOptionsDto refreshDelivery(HttpSession session) {
        ...
        return toDeliveryOptions(member);

    }



    /** Шаг 3: отправка кода на выбранный канал. */
    ...
    public ParentLoginChallengeDto sendCode(LoginCodeChannel channel, HttpSession session) {
        ...
        if (delivery == LoginCodeChannel.MAX) {
            ...
            try {
                ...
                maxSender.sendLoginCode(maxUserId, relationLabel(member.relation()), code);
            ...
            try {
                emailSender.sendLoginCode(email, relationLabel(member.relation()), code);
    ...
    public Optional<ParentDeliveryOptionsDto> pendingDelivery(HttpSession session) {
        ...
        Object challengeRaw = session.getAttribute(PENDING_CHALLENGE_KEY);

        if (challengeRaw instanceof PendingParentChallenge) {
    ...
    public Optional<ParentLoginChallengeDto> pendingChallenge(HttpSession session) {
        ...
        String hint = pending.channel() == LoginCodeChannel.MAX

            ? maskPhone(pending.member().phone())
    ...
    private ParentMemberOptionDto toMemberOption(int index, OneCParentMember parent) {
        ...
        boolean loginAvailable = hasLoginChannel(email, phone);
    ...
    private static String relationLabel(String relation) {
        ...
        String kind = relationKind(relation);

        return switch (kind) {

            case "mother" -> "родитель";

            case "father" -> "родитель";
