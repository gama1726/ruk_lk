...
public class ParentAuthService {
    ...
    public ParentFamilyResponseDto identify(String studentId, HttpSession session) {
        ...
        OneCFamilyResponse family = onecClient.fetchFamily(id)
        ...
        if (!family.parentsFound() || family.parents() == null || family.parents().isEmpty()) {
            throw new ResponseStatusException(
                ...
                "В базе не указаны родители для этого студента"
            );
        }
        ...
        session.setAttribute(PENDING_FAMILY_KEY, new PendingParentFamily(
            family.studentId(),
            family.studentFullName(),
            family.studentAdult(),
            List.copyOf(family.parents())
        ));

        return toFamilyResponse(family);
    }

    public ParentMemberOptionDto selectMember(int memberIndex, HttpSession session) {
        ...
        PendingParentMember pendingMember = new PendingParentMember(
            family.studentId(),
            family.studentFullName(),
            family.studentAdult(),
            memberIndex,
        ...
        session.removeAttribute(PENDING_CHALLENGE_KEY);
        return toMemberOption(memberIndex, parent);
    }

    public ParentLoginChallengeDto sendCode(HttpSession session) {
        ...
        if (email == null || email.isBlank()) {
            if (fixedCode == null || fixedCode.isBlank()) {
                throw new ResponseStatusException(
                    ...
                    "Для входа не указан email родителя в базе университета. Обратитесь в деканат."
                );
            }
            log.warn(
                "Parent login без email: studentId={}, parent={} — используется fixed-code",
                member.studentId(),
        ...
        if (email != null && !email.isBlank()) {
            try {
                emailSender.sendLoginCode(email, member.parentFullName(), code);
        ...
        return new ParentLoginChallengeDto(deliveryHint);
    }
    ...
    public Optional<ParentLoginChallengeDto> pendingChallenge(HttpSession session) {
        ...
        String hint = pending.member().email() != null && !pending.member().email().isBlank()
            ? maskEmail(pending.member().email())
            : "тестовый вход";
        return Optional.of(new ParentLoginChallengeDto(hint));
    }
    ...
    private ParentFamilyResponseDto toFamilyResponse(PendingParentFamily family) {
        ...
        return new ParentFamilyResponseDto(
            family.studentId(),
            family.studentFullName(),
            family.studentAdult(),
            members
    ...
    private ParentFamilyResponseDto toFamilyResponse(OneCFamilyResponse family) {
        List<ParentMemberOptionDto> members = new ArrayList<>();
        List<OneCParentMember> parents = family.parents() == null ? List.of() : family.parents();
        for (int i = 0; i < parents.size(); i++) {
            members.add(toMemberOption(i, parents.get(i)));
        }
        return new ParentFamilyResponseDto(
            family.studentId(),
            family.studentFullName(),
    ...
    private ParentMemberOptionDto toMemberOption(int index, OneCParentMember parent) {
        ...
        return new ParentMemberOptionDto(
            index,
            blankToEmpty(parent.relation()),
            blankToEmpty(parent.fullName()),
            parent.isCustomer(),
