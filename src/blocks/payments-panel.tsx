/**
 * @file UI оплаты обучения (студент и родитель).
 */
...
import styles from '@/pages/payments.module.css'
...
export function PaymentsPanel({ subtitle, fetchPayments, payReturnUrl }: Props) {
  ...
  }, [apiEnabled, fetchPayments])

  if (!apiEnabled) {
    return <PaymentsMock subtitle={subtitle} />
  }

  if (loading) {
    return (
      <>
        <ScreenHeader title="Оплата обучения" subtitle={subtitle} />
        <p>Загрузка…</p>
      </>
    )
  }
  ...
  const onPay = () => {
    ...
    const err = openPayGateway(contract?.number, amount, payReturnUrl)
    setPayError(err)
  }
...
function PaymentsMock({ subtitle }: { subtitle: string }) {
  const p = paymentSummary
  return (
    <>
      <ScreenHeader title="Оплата и договоры" subtitle={subtitle} />
      <p className={styles.warn}>
