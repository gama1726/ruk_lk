/**
 * @file UI приказов (студент и родитель).
 */

import { useEffect, useState } from 'react'
import { ApiError } from '@/apiClient'
import { formatOrderDate, ordersByProgram } from '@/mocks/orders'
import { isOrdersApiEnabled, type StudentOrderItemDto, type StudentOrdersDto } from '@/orders'
...
type Props = {
  subtitle: string
  fetchOrders: () => Promise<StudentOrdersDto>
  mockProgramId?: string
}
...
export function mapMockOrders(programId: string): Row[] {
...
export function OrdersPanel({ subtitle, fetchOrders, mockProgramId }: Props) {
  ...
  useEffect(() => {
    if (!isOrdersApiEnabled()) {
      setRows(mapMockOrders(mockProgramId ?? 'p1'))
      setLoading(false)
      return
    }
